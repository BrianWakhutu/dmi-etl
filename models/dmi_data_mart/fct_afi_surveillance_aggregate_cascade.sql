...
with subset_data as (
select 
         screening_interviewdate,
		 gender as sex,
        CASE When  screening_interviewdate  IS NOT NULL then 1 else 0 end as screened,
...
left join {{ ref( 'dim_date') }} as screen_date on screen_date.date = subset_data.screening_interviewdate

left join {{ ref( 'dim_epi_week') }} as screen_epi_week on subset_data.screening_interviewdate >= screen_epi_week.start_of_week 
	and subset_data.screening_interviewdate <= screen_epi_week.end_of_week 
